...
test('a real view is recorded with a LOCAL date and a local hour', () => {
  const db = fakeStore();
  assert.deepEqual(screens.record('radar', new Date(2026, 8, 17, 14, 30), db), { ok: true });
...
test('the view name is normalised, so a capitalised tab is not a second screen', () => {
  const db = fakeStore();
  assert.deepEqual(screens.record('  Findings  ', new Date(2026, 8, 17, 9), db), { ok: true });
